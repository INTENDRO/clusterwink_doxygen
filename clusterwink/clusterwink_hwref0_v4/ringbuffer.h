...
typedef struct
{
	...
	RingBuff_Count_t Count;					///< Amount of elements currently stored in the ringbuffer
} RingBuff_t;
...
/** Peak at the elements from the ring buffer without removing them.
 *
 *  \param[in,out] Buffer  Pointer to a ring buffer structure to retrieve from
 *  \param[out] Destination  Array to store the data in
 *  \param[in] PeakLength  Amount of elements to be peaked at
 *
 *  \return no return value
 */
...
/** Returns the number of occurrences of a char in the ringbuffer.
 *
 *  \param[in,out] Buffer  Pointer to a ring buffer structure to retrieve from
 *  \param[in] charToCheck  Char to look for
 *
 *  \return no return value
 */
...
/** Removes all elements until a specified char from the ring buffer.
 *
 *  \note Only one execution thread (main program thread or an ISR) may remove from a single buffer
 *        otherwise data corruption may occur. Insertion and removal may occur from different execution
 *        threads.
 ...
 *  \param[in,out] Buffer  Pointer to a ring buffer structure to retrieve from
 *  \param[out] Destination  Array to store the data in
 *  \param[in] endChar  Termination character
 *  \param[in] includingChar  0: Termination not copied to destination  1: Termination char copied to destination
 *  \return no return value
 */
